...
class VisualizationController:
    """Handles visualization data requests."""

    @staticmethod
    def get_curve_data():
        """POST /api/visualization - get curve data for charts."""
        ...
        if depth_min >= depth_max:
            return error_response("depth_min must be less than depth_max", 400)

        try:
            result = VisualizationService.get_curve_data(
                well_id, curve_names, depth_min, depth_max
            )
            if result is None:
